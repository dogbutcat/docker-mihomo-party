...
test-ip-selection: test-build
	...
	@docker run --rm \
		...
		--entrypoint bash $(TEST_IMAGE) -c '\
		set -u && \
		source <(head -12 /etc/s6-overlay/s6-rc.d/init-warp/run | grep -v "^#" | grep -v "^set " | grep -v "exit 0") && \
	...
	@echo ""
	@echo "===== [5/7] init-ip-selection-warp 禁用时跳过 ====="
	@docker run --rm \
		-e WARP=true \
		-e WARP_IP_SELECTION_ENABLED=false \
		-e WARP_API_SELECTION_ENABLED=false \
		--entrypoint bash $(TEST_IMAGE) -c '\
		bash /etc/s6-overlay/s6-rc.d/init-ip-selection-warp/run && \
